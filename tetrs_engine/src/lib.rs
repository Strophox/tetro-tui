...
#[derive(Eq, PartialEq, Ord, PartialOrd, Clone, Copy, Hash, Default, Debug)]
#[cfg_attr(feature = "serde", derive(serde::Serialize, serde::Deserialize))]
pub enum FeedbackVerbosity { // TODO: Implement this.
...
impl GameBuilder {
    ...
    pub fn new(game_mode: GameMode) -> Self {
        GameBuilder { mode: game_mode, config: None, _state: (), rng: None, seed: None, _modifiers: () }
    }

    /// Creates a [`Game`] with the information specified by `self`.
    pub fn build(&self) -> Game {
        self.build_modified(Default::default())
    }

    /// Creates a [`Game`] with the information specified by `self` and some one-time `modifiers`.
    pub fn build_modified(&self, modifiers: Vec<FnGameMod>) -> Game {
        ...
        Game {
            ...
            rng: self.rng.clone().unwrap_or_else(|| GameRng::seed_from_u64(seed)),
            seed,
            modifiers,
        }
    }
