...
pub fn custom_start_board(board_str: &str) -> Modifier {
    ...
    let mod_function: Box<GameModFn> = Box::new(move |_config, _rules, state, _modpoint, _msgs| {
        if !init {
...
pub fn display_tetromino_likelihood() -> Modifier {
    let mod_function: Box<GameModFn> = Box::new(|config, _rules, state, modpoint, msgs| {
    ...
    Modifier {
        name: "display_tetromino_likelihood".to_owned(),
        mod_function,
    }
}
